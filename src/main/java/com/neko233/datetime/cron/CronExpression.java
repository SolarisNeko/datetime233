package com.neko233.datetime.cron;
 ...
 * <p>
 * Borrowed from quartz v2.3.1 <br>
 * <p>
 * Just use in CRON.
 */
public final class CronExpression implements Serializable, Cloneable {
    ...
    protected int storeExpressionVals(int pos,
        ...
        if (c == '*' || c == '/') {
            ...
            if (type == DAY_OF_MONTH && s.length() > i) {
                c = s.charAt(i);
                if (c == '-') {
                    CronPositionValue vs = getValue(0, s, i + 1);
            ...
            if (i >= s.length()) {
                ...
                c = s.charAt(i);
                if (c >= '0' && c <= '9') {
                    CronPositionValue vs = getValue(val, s, i);
                    val = vs.value;
                    i = vs.position;
                }
    ...
    private void checkIncrementRange(int incr,
                                     int type,
                                     int idxPos) throws ParseException {
    ...
    protected int checkNext(int pos,
        ...
        if (c == '-') {
            ...
            c = s.charAt(i);
            if (c >= '0' && c <= '9') {
                CronPositionValue vs = getValue(v, s, i);
                end = vs.value;
                i = vs.position;
            }
            if (i < s.length() && ((c = s.charAt(i)) == '/')) {
                ...
                c = s.charAt(i);
                if (c >= '0' && c <= '9') {
                    CronPositionValue vs = getValue(v2, s, i);
                    int v3 = vs.value;
    ...
    protected CronPositionValue getValue(int v,
                                         String s,
                                         int i) {
        char c = s.charAt(i);
        ...
        CronPositionValue val = new CronPositionValue();

        val.position = (i < s.length()) ? i : i + 1;
